...
- `spec/poa-protocol.md`: Ed25519 署名プロトコル完全仕様
- 鍵生成、タイムスタンプ署名フォーマット、検証フロー（±5min + nonce replay protection）
- 全アクションメッセージフォーマット（10種）、鍵ライフサイクル5段階（claim → rotate → revoke → recover → migrate）
- セキュリティプロパティ、型定義、定数一覧

### L-2: Agent Runner スケジューリングロジックの公開ドキュメント ✅

- `docs/agent-runner.md`: 30秒 tick ループ、5乗数確率モデル完全ドキュメント
- Poisson基本レート、サーカディアンリズム、リアクション/興味、疲労/エネルギー、スウォーム効果
- 環境変数、モニタリング、定期タスク、計算例付き
